...
class IconsBuilder {
  constructor() {
    this.modulePrefix = 'MaterialuiIcons_';
  ...
  async cleanIcons() {
    ...
    let pattern = path.join(this.config.outputDir, 'MaterialuiIcons*.re');
  ...
  async buildParent(parentContents) {
    ...
    let output = path.join(this.config.outputDir + '/MaterialuiIcons.re');
    try {
      await fse.outputFile(output, contents);
      console.log(tag.success + ' builded MaterialuiIcons.re');
    } catch (err) {
      error('failed build MaterialuiIcons.re');
    }
  }
  ...
  async buildIcons() {
    ...
    let buildIcons = fullNames.map(async (fullName) => {
      ...
      console.log(tag.success + ' builded MaterialuiIcons_' + fullName + '.re');
    });
